...
test('file page opens video and PDF previews in new windows without a dialog', () => {
  assert.match(pageSource, /v-else-if=["']isVideoFile\(row\)["']/)
  assert.match(pageSource, /v-else-if=["']isPdfFile\(row\)["']/)
  assert.match(pageSource, /<button[^>]*v-else-if=["']isVideoFile\(row\)["'][^>]*:aria-label=["']`新窗口打开视频：\$\{row\.fileName\}`["'][^>]*@click=["']openFile\(row\.fileUrl\)["'][^>]*>/)
  assert.match(pageSource, /<button[^>]*v-else-if=["']isPdfFile\(row\)["'][^>]*:aria-label=["']`新窗口打开 PDF：\$\{row\.fileName\}`["'][^>]*@click=["']openFile\(row\.fileUrl\)["'][^>]*>/)
  assert.match(pageSource, /<video[\s\S]*?:src=["']row\.fileUrl["'][\s\S]*?muted[\s\S]*?preload=["']metadata["']/)
  assert.match(pageSource, /v-else class=["']file-icon["']/)
  assert.match(pageSource, /const openFile = \(url: string\) => \{\s*window\.open\(url, ['"]_blank['"], ['"]noopener,noreferrer['"]\)\s*\}/)
  assert.doesNotMatch(pageSource, /<video[^>]*\sautoplay(?:\s|=|>)/)
  assert.doesNotMatch(pageSource, /mediaPreview|previewFile|openMediaPreview|pauseMediaPreview|resetMediaPreview/)
  assert.doesNotMatch(pageSource, /<iframe|<video[^>]*\scontrols(?:\s|=|>)/)
  assert.match(pageSource, /\.file-preview-trigger:focus-visible/)
})
