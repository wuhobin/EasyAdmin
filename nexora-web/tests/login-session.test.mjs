...
test('profile redesign preserves account editing and avatar upload behavior', () => {
  assert.match(profileSource, /class="profile-layout"/)
  assert.match(profileSource, /class="identity-card"/)
  assert.match(profileSource, /class="settings-card"/)
  ...
  assert.match(profileSource, /height:\s*100%;[\s\S]*min-height:\s*0;[\s\S]*overflow:\s*hidden;/)
  assert.match(profileSource, /@media \(max-width: 960px\)[\s\S]*height:\s*auto;[\s\S]*overflow:\s*visible;/)
  assert.doesNotMatch(profileSource, /min-height:\s*640px/)
  assert.match(profileSource, /\.profile-layout\s*\{[\s\S]*align-items:\s*start;/)
  assert.doesNotMatch(profileSource, /\.settings-card\s*\{[^}]*height:\s*100%;/)
})
...
test('package scripts expose repeatable test, typecheck, and full verification commands', () => {
  assert.equal(
    ...
    'node --test tests/login-page.test.mjs tests/login-session.test.mjs tests/file-management.test.mjs tests/mail-inbox.test.mjs'
  )
