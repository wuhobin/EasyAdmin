...
function MenuItem({ route, depth = 0, activePath, collapsed }: { route: MenuRoute; depth?: number; activePath: string; collapsed: boolean }) {
  ...
  const activate = () => {
    if (route.meta.isExternal || isExternalPath(route.fullPath)) {
  ...
  return (
    <SidebarMenuItem className={`depth-${depth} ${selected ? 'is-selected' : ''}`}>
      <SidebarMenuButton onClick={activate} isActive={selected} tooltip={collapsed ? route.meta.title : undefined} title={collapsed ? route.meta.title : undefined} aria-current={activePath === route.fullPath ? 'page' : undefined}>
        <MenuIcon value={route.meta.icon} className="nav-item-icon" />
        {!collapsed ? <span className="nav-item-title">{route.meta.title}</span> : null}
        {!collapsed && hasChildren ? <span className={`nav-item-chevron ${open ? 'is-open' : ''}`}>⌄</span> : null}
      </SidebarMenuButton>
      {hasChildren && open && !collapsed ? <SidebarMenuSub>{children.map(child => <MenuItem key={`${child.id}-${child.fullPath}`} route={child} depth={depth + 1} activePath={activePath} collapsed={collapsed} />)}</SidebarMenuSub> : null}
    </SidebarMenuItem>
  )
}
...
function SidebarUser() {
  ...
  const items = [
    { key: 'profile', icon: <UserOutlined />, label: '个人中心', onClick: () => navigate(PROFILE_PATH) },
  ...
  return (
      ...
      popupRender={menu => (
        <div className="sidebar-account-popover">
          <div className="sidebar-account-summary">
            <span className="sidebar-account-avatar">{user.avatar ? <img src={user.avatar} alt="" /> : nickname.slice(0, 1).toUpperCase()}</span>
    ...
    >
      <button className="sidebar-user" type="button" aria-label={`${nickname}，打开账户菜单`} title={collapsed ? nickname : undefined}>
        <span className="sidebar-user-avatar">{user.avatar ? <img src={user.avatar} alt="" /> : nickname.slice(0, 1).toUpperCase()}</span>
