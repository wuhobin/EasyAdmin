...
export function ProfilePage() {
  ...
  const emailMutation = useMutation({
    mutationFn: (values: EmailFormValues) => { const normalized = normalizeEmailForm(values); return changeEmailApi(normalized.email, normalized.code) },
    onSuccess: async () => {
      await refreshIdentity()
      setEmailDialogOpen(false)
      emailForm.reset(EMPTY_EMAIL_FORM)
      setCodeCountdown(0)
      message.success('登录邮箱已更新')
    },
  ...
  return (
    <section className="profile-page">
          ...
          <div className="profile-panel" role="tabpanel">
            ...
            <Form {...profileForm}>
              <form className="profile-settings-form" onSubmit={profileForm.handleSubmit(values => profileMutation.mutate(values))}>
                ...
                <div data-slot="form-item" className="profile-readonly-field"><label htmlFor="profile-current-email">登录邮箱</label><div className="profile-email-setting"><Input id="profile-current-email" value={profile.email} disabled /><Button type="button" variant="outline" onClick={() => { emailForm.reset(EMPTY_EMAIL_FORM); setEmailDialogOpen(true) }}><MailOutlined />更换邮箱</Button></div></div>
      ...
      <Dialog open={emailDialogOpen} onOpenChange={open => { setEmailDialogOpen(open); if (!open) { emailForm.reset(EMPTY_EMAIL_FORM); setCodeCountdown(0) } }}>
        <DialogContent className="profile-email-dialog max-w-[500px]">
          <Form {...emailForm}>
            <form onSubmit={emailForm.handleSubmit(values => emailMutation.mutate(values))}>
              <DialogHeader><DialogTitle>更换登录邮箱</DialogTitle><DialogDescription className="profile-email-dialog-description">验证码将发送到新邮箱，验证通过后用于后续登录。</DialogDescription></DialogHeader>
              ...
              <DialogFooter><DialogClose asChild><Button type="button" variant="outline">取消</Button></DialogClose><Button type="submit" loading={emailMutation.isPending}>确认更换</Button></DialogFooter>
            </form>
          </Form>
        </DialogContent>
      </Dialog>
