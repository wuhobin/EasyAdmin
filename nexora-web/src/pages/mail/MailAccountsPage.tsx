...
export function MailAccountsPage() {
  ...
  const [filters, setFilters] = useState<AccountFilterValues>({ keyword: '', provider: 'all', status: 'all' })
  ...
  const columns: ColumnsType<MailAccount> = [
    ...
    { title: '最后连接', dataIndex: 'lastConnectTime', width: 176, align: 'center', render: value => value || <span className="management-empty-value">-</span> },
    ...
    <section className="management-page mail-accounts-page">
      ...
      <Sheet open={Boolean(detailAccount)} onOpenChange={open => { if (!open) setDetailAccount(undefined) }}>
        <SheetContent className="w-[min(430px,96vw)]">
          ...
          <SheetBody>{detailAccount ? <div className="mail-account-detail"><header><span className={`mail-provider-avatar large ${providerClass(detailAccount.provider)}`}>{providerMark(detailAccount.provider)}</span><div><h3>{detailAccount.accountName}</h3><p>{detailAccount.email}</p></div></header><dl><div><dt>邮箱类型</dt><dd>{providerLabel(providers, detailAccount.provider)}</dd></div><div><dt>账户状态</dt><dd>{detailAccount.enabled === 1 ? '已启用' : '已停用'}</dd></div><div><dt>排序</dt><dd>{detailAccount.sort}</dd></div><div><dt>最后连接</dt><dd>{detailAccount.lastConnectTime || '尚未连接'}</dd></div><div><dt>连接结果</dt><dd className={detailAccount.lastError ? 'error' : 'success'}>{detailAccount.lastError || (detailAccount.lastConnectTime ? '连接正常' : '尚未测试')}</dd></div></dl><p className="mail-account-security"><LockOutlined />授权码已经加密保存，不会在详情或编辑接口中返回。</p></div> : null}</SheetBody>
        </SheetContent>
      </Sheet>
