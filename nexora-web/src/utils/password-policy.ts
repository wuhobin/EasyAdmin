...
export function passwordPolicyDescription(policy: PasswordConfig) {
  const requirements = [
...
export function validatePasswordByPolicy(password: string, policy: PasswordConfig): string | null {
  if (!password) return '请输入密码'
  const length = Array.from(password).length
  if (length < policy.minLength || length > policy.maxLength) {
    return `密码长度必须在 ${policy.minLength} 到 ${policy.maxLength} 个字符之间`
  }
  if (new TextEncoder().encode(password).length > 72) return '密码 UTF-8 编码不能超过 72 字节'
  ...
  if (policy.requireSpecial && !SPECIAL_CHARACTER_PATTERN.test(password)) return '密码必须包含特殊字符'
  return null
}
