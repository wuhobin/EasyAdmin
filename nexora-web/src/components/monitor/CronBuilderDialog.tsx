      ...
      <DialogContent className="cron-dialog max-w-[760px]">
        ...
        <div className="cron-builder">
          ...
          <section className="cron-rule-panel" aria-label={`${config.label}规则`}>
            ...
            {selected.mode === 'cycle' ? <div className="cron-cycle-editor"><span>从</span><Input name={`cron-${activeUnit}-start`} aria-label={`${config.label}循环起始值`} type="number" inputMode="numeric" min={config.min} max={config.max} value={selected.start} onChange={event => updateUnit({ start: Number(event.target.value) })} /><span>{config.unit}开始，每</span><Input name={`cron-${activeUnit}-interval`} aria-label={`${config.label}循环间隔`} type="number" inputMode="numeric" min={1} max={Math.max(1, config.max)} value={selected.interval} onChange={event => updateUnit({ interval: Number(event.target.value) })} /><span>{config.unit}执行一次</span></div> : null}
