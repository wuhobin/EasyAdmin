...
export function getForceLogoutConfirmation(session: OnlineSessionRecord) {
  const target = getSessionUserLabel(session)
  const sessionId = abbreviateSessionId(session.sessionId)
  const redirectNotice = session.currentSession
    ? ' 这是当前会话，操作成功后将返回登录页。'
    : ''
  return `确定强退用户“${target}”的会话 ${sessionId} 吗？${redirectNotice}`
}
...
export async function runForceLogoutFlow(
  session: OnlineSessionRecord,
  dependencies: ForceLogoutFlowDependencies
): Promise<ForceLogoutFlowResult> {
  try {
  ...
  const pageState = await dependencies.refresh()
  const fallbackPage = resolveFallbackPage(pageState)
  if (fallbackPage !== undefined && fallbackPage !== pageState.pageNum) {
    await dependencies.refresh(fallbackPage)
  }
  return 'refreshed'
}
