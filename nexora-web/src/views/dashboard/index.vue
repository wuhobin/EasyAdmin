<template>
  <div class="dashboard-container">
    ...
    <section class="chart-grid"><el-card shadow="never"><template #header><div class="card-title"><span>访问趋势</span><small>近七日</small></div></template><div ref="lineChartRef" class="chart" role="img" aria-label="近七日访问量和浏览量折线趋势图"></div></el-card><el-card shadow="never"><template #header><div class="card-title"><span>分类统计</span><small>当前内容分布</small></div></template><div ref="pieChartRef" class="chart" role="img" aria-label="内容分类占比环形图"></div></el-card></section>
  </div>
</template>
...
const initCharts = () => { if (lineChartRef.value && !lineChart.value) { lineChart.value = echarts.init(lineChartRef.value) } if (pieChartRef.value && !pieChart.value) { pieChart.value = echarts.init(pieChartRef.value) } lineChart.value?.setOption(chartOption()); pieChart.value?.setOption(pieOption()) }
let resizeTimer: ReturnType<typeof setTimeout> | undefined
const resize = () => { clearTimeout(resizeTimer); resizeTimer = setTimeout(() => { lineChart.value?.resize(); pieChart.value?.resize() }, 150) }
onMounted(() => { initCharts(); window.addEventListener('resize', resize) }); onUnmounted(() => { clearTimeout(resizeTimer); window.removeEventListener('resize', resize); lineChart.value?.dispose(); pieChart.value?.dispose() })
</script>
