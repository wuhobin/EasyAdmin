  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button id={id} type="button" variant="outline" role="combobox" aria-expanded={open} aria-invalid={invalid || undefined} disabled={disabled || loading} className={cn('h-auto min-h-9 w-full justify-between px-3 py-1.5 font-normal focus-visible:ring-0 focus-visible:ring-offset-0', !selected.length && 'text-[var(--nexora-placeholder)]')}>
      ...
      <PopoverContent align="start" className="w-[var(--radix-popover-trigger-width)] p-2">
        <div className="relative mb-2">
          <Search className="pointer-events-none absolute left-2.5 top-1/2 size-4 -translate-y-1/2 text-[var(--nexora-placeholder)]" />
          <Input value={keyword} onChange={event => setKeyword(event.target.value)} placeholder={searchPlaceholder} className="pl-8" />
        </div>
